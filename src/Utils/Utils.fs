...
module Binary =
    ...
    let binStrToDec i = Convert.ToInt64(i, 2)
    let binIntToDec i = i |> List.map string |> String.concat "" |> binStrToDec

    let negate (bin: int list) =
        bin
        |> List.map (function
            | 0 -> 1
            | 1 -> 0
            | i -> failwithf "Not a binary %A" i
        )
